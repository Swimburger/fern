...
class ServiceClient
{
    ...
    * GET request with custom api key
    ...
    {
        try {
            $response = $this->client->sendRequest(new JsonApiRequest(baseUrl: $this->options['baseUrl'] ?? '', path: "apiKey", method: HttpMethod::GET));
    ...
    * GET request with custom api key
    ...
    public function getWithHeader(HeaderAuthRequest $request, ?array $options = null): mixed
    {
        ...
        try {
            $response = $this->client->sendRequest(new JsonApiRequest(baseUrl: $this->options['baseUrl'] ?? '', path: "apiKeyInHeader", method: HttpMethod::GET, headers: $headers));
            $statusCode = $response->getStatusCode();
            if ($statusCode >= 200 && $statusCode < 400) {
                return json_decode($response->getBody()->getContents(), true, 512, JSON_THROW_ON_ERROR);
            }

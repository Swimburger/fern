...
class ParamsClient
{
    ...
    * GET with path param
    ...
    {
        try {
            $response = $this->client->sendRequest(new JsonApiRequest(baseUrl: $this->options['baseUrl'] ?? '', path: "/params/path/$$param", method: HttpMethod::GET));
    ...
    * GET with query param
    ...
    public function getWithQuery(GetWithQuery $request, ?array $options = null): mixed
    {
        ...
        try {
            $response = $this->client->sendRequest(new JsonApiRequest(baseUrl: $this->options['baseUrl'] ?? '', path: "/params", method: HttpMethod::GET, query: $query));
            $statusCode = $response->getStatusCode();
            if ($statusCode >= 200 && $statusCode < 400) {
                return;
            }
    ...
    * GET with path and query params
    ...
    public function getWithPathAndQuery(string $param, GetWithPathAndQuery $request, ?array $options = null): mixed
    {
        ...
        $query['query'] = $request->query;
        try {
            $response = $this->client->sendRequest(new JsonApiRequest(baseUrl: $this->options['baseUrl'] ?? '', path: "/params/path-query/$$param", method: HttpMethod::GET, query: $query));
            $statusCode = $response->getStatusCode();
            if ($statusCode >= 200 && $statusCode < 400) {
                return;
            }
    ...
    * PUT to update with path param
    ...
    {
        try {
            $response = $this->client->sendRequest(new JsonApiRequest(baseUrl: $this->options['baseUrl'] ?? '', path: "/params/path/$$param", method: HttpMethod::PUT, body: $request));
            $statusCode = $response->getStatusCode();
            if ($statusCode >= 200 && $statusCode < 400) {
                return json_decode($response->getBody()->getContents(), true, 512, JSON_THROW_ON_ERROR);
            }

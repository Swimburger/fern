...
class ServiceClient
{
    ...
    {
        try {
            $response = $this->client->sendRequest(new JsonApiRequest(baseUrl: $this->options['baseUrl'] ?? '', path: "/resource/$$resourceId", method: HttpMethod::GET));
    ...
    public function listResources(ListResourcesRequest $request, ?array $options = null): mixed
    {
        ...
        try {
            $response = $this->client->sendRequest(new JsonApiRequest(baseUrl: $this->options['baseUrl'] ?? '', path: "/resource", method: HttpMethod::GET, query: $query));
            $statusCode = $response->getStatusCode();
            if ($statusCode >= 200 && $statusCode < 400) {
                return json_decode($response->getBody()->getContents(), true, 512, JSON_THROW_ON_ERROR);
            }

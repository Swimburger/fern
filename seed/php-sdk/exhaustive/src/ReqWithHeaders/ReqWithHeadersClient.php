...
class ReqWithHeadersClient
{
    ...
    public function getWithCustomHeader(ReqWithHeaders $request, ?array $options = null): mixed
    {
        ...
        try {
            $response = $this->client->sendRequest(new JsonApiRequest(baseUrl: $this->options['baseUrl'] ?? '', path: "/test-headers/custom-header", method: HttpMethod::POST, headers: $headers, body: $request->body));

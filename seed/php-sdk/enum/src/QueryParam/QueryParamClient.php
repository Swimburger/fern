...
class QueryParamClient
{
    ...
    public function send(SendEnumAsQueryParamRequest $request, ?array $options = null): mixed
    {
        ...
        try {
            $response = $this->client->sendRequest(new JsonApiRequest(baseUrl: $this->options['baseUrl'] ?? '', path: "query", method: HttpMethod::POST, query: $query));
    ...
    public function sendList(SendEnumListAsQueryParamRequest $request, ?array $options = null): mixed
    {
        ...
        try {
            $response = $this->client->sendRequest(new JsonApiRequest(baseUrl: $this->options['baseUrl'] ?? '', path: "query-list", method: HttpMethod::POST, query: $query));
            $statusCode = $response->getStatusCode();
            if ($statusCode >= 200 && $statusCode < 400) {
                return;
            }
